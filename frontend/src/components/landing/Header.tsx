      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5" aria-label="Degravar.adv.br · página inicial">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-navy-900 text-white shadow-sm">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-[0.92rem] font-medium text-slate-600 hover:text-navy-900 transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          <button
            ...
            aria-expanded={open}
            className="md:hidden -mr-2 p-2 text-slate-700 hover:text-navy-900 transition-colors"
          >
        ...
        >
          <nav className="flex flex-col py-2 border-t border-slate-100">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                onClick={() => setOpen(false)}
                className="py-3 text-[0.95rem] font-medium text-slate-700 hover:text-navy-900"
              >
                {link.label}
              </a>
            ))}
