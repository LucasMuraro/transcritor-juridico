    ...
    <section id="como-funciona" className="py-20 lg:py-28 bg-white">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="text-center mb-14 lg:mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-[2.6rem] font-extrabold tracking-tight text-ink-900 text-balance">
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="relative">
              <div className="flex items-center gap-3 mb-5">
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-ink-900 text-white text-[0.95rem] font-extrabold tracking-tight">
                  0{i + 1}
                </span>
              ...
              <h3 className="text-[1.25rem] font-bold text-ink-900 mb-2 tracking-tight">
                {step.title}
              </h3>
              <p className="text-[0.95rem] text-ink-500 leading-relaxed">
                {step.description}
              </p>

              {i < steps.length - 1 && (
                <div
                  className="hidden md:block absolute top-[22px] left-[calc(100%-1.5rem)] w-[3rem] h-px bg-gradient-to-r from-paper-300 to-transparent"
                  aria-hidden
                />
              )}
            </div>
